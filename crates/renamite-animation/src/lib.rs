...
impl FrameRate {
    pub fn fps(self) -> f64 { self.num as f64 / self.den.max(1) as f64 }
    pub fn secs_to_frames(self, secs: f64) -> f64 { secs * self.fps() }
    pub fn frames_to_secs(self, frames: f64) -> f64 { frames / self.fps() }
}

#[derive(Clone, Copy, Debug, PartialEq, Eq, Serialize, Deserialize)]
pub enum Interpolation { Hold, Linear, CubicBezier }

#[derive(Clone, Copy, Debug, PartialEq, Serialize, Deserialize)]
pub struct EasingHandle { pub x: f64, pub y: f64 }

impl EasingHandle {
    pub const LINEAR_OUT: Self = Self { x: 1.0 / 3.0, y: 1.0 / 3.0 };
    pub const LINEAR_IN: Self = Self { x: 2.0 / 3.0, y: 2.0 / 3.0 };
    pub fn clamped(self) -> Self { Self { x: self.x.clamp(0.0, 1.0), y: self.y } }
}
...
#[derive(Clone, Copy, Debug, PartialEq, Eq, Serialize, Deserialize)]
pub enum EasingPreset { Linear, EaseIn, EaseOut, EaseInOut, Anticipate, Overshoot, Hold }
...
impl EasingPreset {
    ...
    pub fn segment(self) -> (Interpolation, EasingHandle, EasingHandle) {
        ...
        match self {
            Linear => (Interpolation::Linear, EasingHandle::LINEAR_OUT, EasingHandle::LINEAR_IN),
            ...
            Hold => (Interpolation::Hold, EasingHandle::LINEAR_OUT, EasingHandle::LINEAR_IN),
        }
    }
    ...
    pub fn next(self) -> Self {
        use EasingPreset::*;
        match self {
            Linear => EaseIn, EaseIn => EaseOut, EaseOut => EaseInOut,
            EaseInOut => Anticipate, Anticipate => Overshoot, Overshoot => Hold, Hold => Linear,
        }
    }
    ...
    pub fn detect(i: Interpolation, o: EasingHandle, e: EasingHandle) -> Option<Self> {
        use EasingPreset::*;
        let eq = |a: EasingHandle, b: EasingHandle| (a.x - b.x).abs() < 1e-6 && (a.y - b.y).abs() < 1e-6;
        for p in [Linear, EaseIn, EaseOut, EaseInOut, Anticipate, Overshoot, Hold] {
            let (pi, po, pe) = p.segment();
            if pi == i && (i == Interpolation::Hold || (eq(po, o) && eq(pe, e))) { return Some(p); }
        }
        None
    }
}
...
pub enum KeyEdit { Added, Updated }
...
impl<T: Tween> Animated<T> {
    pub fn new(base: T) -> Self { Self { base, keyframes: Vec::new() } }

    pub fn value_at(&self, frame: f64) -> T {
        let ks = &self.keyframes;
        if ks.is_empty() { return self.base.clone(); }
        if frame <= ks[0].frame.0 as f64 { return ks[0].value.clone(); }
        let last = ks.len() - 1;
        if frame >= ks[last].frame.0 as f64 { return ks[last].value.clone(); }
    ...
    pub fn set_key(&mut self, frame: Frame, value: T) -> KeyEdit {
        match self.keyframes.binary_search_by_key(&frame, |k| k.frame) {
            Ok(i) => { self.keyframes[i].value = value; KeyEdit::Updated }
            Err(i) => {
                let (interpolation, ease_out, ease_in) = EasingPreset::Linear.segment();
                self.keyframes.insert(i, Keyframe { frame, value, interpolation, ease_out, ease_in });
                KeyEdit::Added
            }
        }
    }
    ...
    pub fn move_key(&mut self, from: Frame, to: Frame) -> bool {
        ...
        if self.key_at(to).is_some() { return false; }
        let Some(mut k) = self.remove_key(from) else { return false };
        k.frame = to;
    ...
    pub fn set_easing(
        &mut self, frame: Frame, interpolation: Interpolation,
        ease_out: EasingHandle, ease_in: EasingHandle,
    ) -> Option<(Interpolation, EasingHandle, EasingHandle)> {
        let i = self.keyframes.binary_search_by_key(&frame, |k| k.frame).ok()?;
    ...
    pub fn has_keys(&self) -> bool { !self.keyframes.is_empty() }
    pub fn key_at(&self, frame: Frame) -> Option<&Keyframe<T>> {
        self.keyframes.binary_search_by_key(&frame, |k| k.frame).ok().map(|i| &self.keyframes[i])
    }
}
...
fn solve_cubic_x(x1: f64, x2: f64, u: f64) -> f64 {
    ...
    for _ in 0..32 {
        t = 0.5 * (lo + hi);
        if cubic_at(t, x1, x2) < u { lo = t } else { hi = t }
    }
    t
}
...
impl Tween for f64 {
    fn tween(a: &Self, b: &Self, t: f64) -> Self { a + (b - a) * t }
}
...
impl Tween for Angle {
    fn tween(a: &Self, b: &Self, t: f64) -> Self { Angle(a.0 + (b.0 - a.0) * t) }
}
...
impl Tween for VectorPath {
    fn tween(a: &Self, b: &Self, t: f64) -> Self {
        ...
        VectorPath {
            closed: a.closed,
            anchors: a.anchors.iter().zip(&b.anchors).map(|(x, y)| Anchor {
...
#[derive(Clone, Copy, Debug, PartialEq, Eq, Serialize, Deserialize)]
pub enum PlayState { Stopped, Playing, Scrubbing }
#[derive(Clone, Copy, Debug, PartialEq, Eq, Serialize, Deserialize)]
pub enum LoopMode { Once, Loop, PingPong }
...
impl Playback {
    pub fn stopped(range: (Frame, Frame)) -> Self {
        Self { state: PlayState::Stopped, head: range.0.0 as f64, loop_mode: LoopMode::Loop, range, dir: 1.0 }
    }

    /// Returns true if the visual head moved (redraw without doc dirty).
    pub fn advance(&mut self, dt_secs: f64, rate: FrameRate) -> bool {
        if self.state != PlayState::Playing { return false; }
        let (s, e) = (self.range.0.0 as f64, self.range.1.0 as f64);
        if e <= s { return false; }
        self.head += self.dir * dt_secs * rate.fps();
        match self.loop_mode {
            LoopMode::Once => {
                if self.head >= e { self.head = e; self.state = PlayState::Stopped; }
                if self.head < s { self.head = s; }
            }
            LoopMode::Loop => { self.head = s + (self.head - s).rem_euclid(e - s); }
            LoopMode::PingPong => {
                let mut guard = 0;
                while (self.head > e || self.head < s) && guard < 8 {
                    if self.head > e { self.head = 2.0 * e - self.head; self.dir = -self.dir; }
                    if self.head < s { self.head = 2.0 * s - self.head; self.dir = -self.dir; }
                    guard += 1;
                }
            }
        }
...
mod tests {
    ...
    fn hold_holds() {
        let mut a = anim();
        a.set_easing(Frame(0), Interpolation::Hold, EasingHandle::LINEAR_OUT, EasingHandle::LINEAR_IN);
    ...
    #[test]
    fn preset_roundtrip() {
        for p in [EasingPreset::Linear, EasingPreset::EaseInOut, EasingPreset::Hold] {
            let (i, o, e) = p.segment();
